...
#define LOCK(q) (q)->lock.lock();
#define UNLOCK(q) (q)->lock.unlock();
...
void link_free(link_list *l)
{
	timer_node *p = l->head.next;
	while (p) {
		timer_node *tmp = p->next;
		skynet_free(p);
		p = tmp;
	}
}

void skynet_timer_exit(void)
{
	for (int i = 0; i < TIME_NEAR; ++i) {
		link_free(&TI->near_[i]);
	}
